...
class Race_DB {
  ...
  create(raceName, raceDesc) {
    return new Promise((res, ret) => db.run(`INSERT INTO Races VALUES (${null}, "${raceName}", "${raceDesc}", 0, 0, 0, 0, 0, 0)`, function(err) {
      if (err)
        ret(err)
    }));
  ...
  getAll() {
    return new Promise((res, ret) => db.all(`SELECT * FROM Races`, function(err, row) {
      if (err)
        ret(err)
      else
        res(row)
